...
class Cupons extends Model
{
	...
	{
		$array = array();
		$sql ="SELECT * FROM cupons WHERE status = 0";
	...
	{
		$array = array();
		$sql ="SELECT * FROM cupons WHERE status = 1";
		$sql = $this->db->query($sql);
		$sql->execute();
	...
	public function saveCupon($cupon)
	{
		$sql ="INSERT INTO cupons SET valor_cupon = :vc";
		$sql = $this->db->prepare($sql);
		$sql->bindValue(':vc',$cupon);
		$sql->execute();
		...
		if ($sql->rowCount() > 0) {
			$_SESSION['msg'] = "Swal.fire({
                                  position: 'center',
                                  type: 'success', 
                                  title: 'Cupon Cadastrado com Sucesso!',
	...
	public function editCupon($cupon, $id)
	{
		$sql ="UPDATE cupons SET valor_cupon = :vc WHERE id_cupon = :id";
		$sql = $this->db->prepare($sql);
		$sql->bindValue(':vc',$cupon);
		$sql->bindValue(':id',$id);
		$sql->execute();
		...
		if ($sql->rowCount() > 0) {
			$_SESSION['msg'] = "Swal.fire({
                                  position: 'center',
                                  type: 'success', 
                                  title: 'Cupon Editado com Sucesso!',
                                  showConfirmButton: false,
                                  timer: 2500
                                })";
		} else {
	...
	public function toggleStatus($id_cupon)
	{
		$sql ="UPDATE cupons SET status = 1 - status WHERE id_cupon = '$id_cupon'";
		$sql = $this->db->query($sql);
